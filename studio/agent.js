 ...
 * across every game, spots patterns, coaches your weak spots, and generates
 * a daily challenge tailored to you. No AI key required — pure rules, so it
 * runs fully local. (A future owner-gated upgrade can swap in an LLM for
 * richer commentary; the interface is the same window.Agent.chat().)
 * ========================================================================= */
(function () {
  'use strict';
  ...
  var TIPS = {
    pinball: {
      drain_left: "You're draining left a lot. The left rescue bumper cluster (LVL 1) punches balls back to center — and aim for the inlane, not the outlane gap.",
      ...
      low_combo: "Your combos fizzle. Stop swinging at random — pick ONE chain: ramp → drops → ramp. Three different shots in 4 seconds is a named combo every time.",
    ...
    darts: {
      ...
      no_180s: "You've yet to throw a 180. Three straight treble-20s. Line up the reticle high and trust the flick — stop aiming for the bull as a crutch.",
    ...
    orbital: {
      ...
      low_waves: "You're not reaching high waves. Let drones clump, then chain-kill for combo multiplier. Shield + rapid-fire pickup timing is everything.",
  ...
  // Build a normalized stats snapshot across all games.
  function snapshot() {
    var g = S.games();
    var pin = g.pinball || {};
    var dar = g.darts || {};
  ...
  function weakness() {
    ...
    if ((sn.pin.plays || 0) > 2 && (sn.pin.total || 0) / Math.max(1, sn.pin.plays) < 8000)
      candidates.push({ game: 'pinball', dim: 'low_combo', score: 0.8 });
    if ((sn.dar.plays || 0) > 2 && (sn.dar.total || 0) / Math.max(1, sn.dar.plays) < 2000)
      candidates.push({ game: 'darts', dim: 'too_soft', score: 0.7 });
    if ((sn.orb.plays || 0) > 1 && (sn.orb.best || 0) < 5000)
      candidates.push({ game: 'orbital', dim: 'die_early', score: 0.75 });
    if ((sn.puz.plays || 0) > 1 && (sn.puz.total || 0) / Math.max(1, sn.puz.plays) < 3)
      candidates.push({ game: 'puzzles', dim: 'par_hog', score: 0.6 });
    // generic fallbacks by play volume
    if (!sn.pin.plays) candidates.push({ game: 'pinball', dim: 'no_secrets', score: 0.9 });
    if (!sn.dar.plays) candidates.push({ game: 'darts', dim: 'checkout_blind', score: 0.9 });
    candidates.sort(function (a, b) { return b.score - a.score; });
    return candidates[0] || { game: 'pinball', dim: 'low_combo', score: 0.5 };
  }

  function coachText(w) {
    var gameTips = TIPS[w.game] || {};
    return gameTips[w.dim] || "Keep playing — the more data I get, the sharper my reads.";
  }

  // Daily challenge generator — deterministic per day so it feels real.
  function dailyChallenge() {
    var sn = snapshot();
    var day = Math.floor(Date.now() / 86400000);
    var pool = [];
    pool.push({ game: 'pinball', text: 'Rip a TRIPLE LOOP in one ball (all 3 ramps).', xp: 150 });
    pool.push({ game: 'pinball', text: 'Light all 4 secret pockets in a single game.', xp: 200 });
    pool.push({ game: 'darts',   text: 'Throw a 180 (three straight treble-20s).', xp: 150 });
    pool.push({ game: 'darts',   text: 'Finish a 501 leg with a checkout over 80.', xp: 200 });
    pool.push({ game: 'orbital', text: 'Reach wave 10 with a 4x combo active.', xp: 180 });
    pool.push({ game: 'puzzles', text: 'Three-star any 6x6 board.', xp: 150 });
    pool.push({ game: 'any',     text: 'Score in 3 different games today.', xp: 250 });
    pool.push({ game: 'pinball', text: 'Go a full game without tilting.', xp: 120 });
  ...
  // Trend: compare last N events to the N before that. Returns +1 / 0 / -1.
  ...
  function summary() {
    var sn = snapshot();
    var lines = [];
    lines.push('Level ' + sn.level + ' · ' + sn.xp.toLocaleString() + ' XP · ' + sn.totPlays + ' total plays across the studio.');
    var fav = '—', favN = 0;
  ...
  window.Agent = {
    ...
    trend: trend,
    summary: summary,
    // Render a full coaching card. Returns {title, body, game, dim}.
    card: function () {
      var w = weakness();
      return { title: w.game.toUpperCase() + ' · COACHING', body: coachText(w),
               game: w.game, dim: w.dim, score: w.score };
    },
    // Lightweight query for a chat-style UI. Extensible to LLM later.
    chat: function (msg) {
      msg = (msg || '').toLowerCase();
      ...
      if (/trend|better|worse|improving/.test(msg)) {
        var t = trend('pinball');
        return t === 1 ? "You're trending UP on pinball — scores climbing."
             : t === -1 ? "Pinball scores dipping. Focus on the coaching tip."
             : "Pinball's holding steady. Small gains — lock in a combo chain.";
      }
