...
|---|---|---|---|---|---|---|---|---|
| DBZ-036 | Devbiz | TBD | `claude/website-pages-audit-r59w6n` | docs | `DRAFT` | Owner review of the audit | Review & merge; decide which backlog items to action next | Full website page-by-page audit (`docs/WEBSITE_AUDIT.md`) + discoverability correction: added the three complete, publicly-linked games (Riff, Riff II, Pinball) to `sitemap.xml` and `llms.txt`, which previously omitted them. Documents owner-gated gaps (social URLs, contact backend, AI concierge key, domain) and unfinished games (Voidrunner, Contra) as a ranked backlog — none actioned. Bundles a docs + SEO/content lane at owner direction; split-able. |
