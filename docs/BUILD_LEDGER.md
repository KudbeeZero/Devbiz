...
|---|---|---|---|---|---|---|---|---|
| DBZ-182 | Devbiz | #182 | `claude/games-page-professional-polish` | feature | `DRAFT` | Manual preview | Browser review of games section visual polish | **PR182: Games page professional polish** — Comprehensive CSS enhancements for "$1M development team" quality. Game cards: enhanced shadows (multi-layer depth), color-coded gradient overlays, improved badges with glow animations, expanded play button (56px, 1.18 scale hover). Section: improved typography hierarchy, animated kicker/title, better spacing. Hero: 40px+100px shadows, light overlays, animated scanlines. League: backdrop blur, accent glow, responsive padding. Buttons: 12px radius, 18px padding, smooth transitions. Responsive: tablet 2-col, mobile 1-col. Local verification: `npm run verify` — 105/105 passed. All changes CSS-only; no game logic affected. Ready for visual review. |
| DBZ-079 | Devbiz | #183 | `claude/pinball-ball-physics-8khnuy` | bugfix | `DRAFT` | Manual browser play-test on preview | Play-test rolling / center drain / cradle; approve merge | **Pinball stuck-ball recovery & enhanced nudge mechanics (PR #183):** Stuck-ball detection system with three-layer recovery (bounds checking for left/right/ceiling, corner-trap timer, enhanced nudge with adaptive force). Bounds: x < 30 or x > 870 with speed < 50 px/s triggers outward kick; y < 80 with speed < 60 px/s triggers downward push; corner trap (y < 300, x < 200, speed < 40 px/s) uses 2-second timer for randomized escape. Enhanced nudge: detects slow balls (speed < 80 px/s), applies 1.6x multiplier on horizontal force, adds -200 additional downward force. All recovery methods trigger burst particles and "FREED" callouts. Evaluator 21/21, local passed, no CI configured. |
