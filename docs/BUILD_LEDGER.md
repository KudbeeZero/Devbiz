...
| DBZ-065 | Devbiz | N/A | `claude/phase-c-leaderboard-wiring` | feature | `DRAFT` | Browser preview | — | **Phase C — Wire online leaderboard SDKs for contra, munch, orbital, puzzles**: Add `GAMES` entries (contra: score/bestCombo/waves/kills; munch: score/level/chipsEaten; orbital: score/wave/kills; puzzles: boardsSolved/bestMoves/flawlessStreak) to `leaderboard/shared/core.js`; add schema columns + indexes to `leaderboard/worker/schema.sql`; wire `KD_LB_CONFIG` + SDK script + `_lbPost` in each game. Demo mode works immediately. |
