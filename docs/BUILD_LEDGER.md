...
|---|---|---|---|---|---|---|---|---|
| DBZ-060 | Devbiz | TBD | N/A | CI/tests | `PLAN` | N/A | — | **Coverage-gate flake, mechanism confirmed 2026-07-07 via PR #144's CI log; earlier JWKS hypothesis was WRONG.** The `leaderboard/shared/ coverage gate` job runs two steps (`.github/workflows/coverage.yml`) — (1) `npm run coverage` (`c8 --check-coverage` against `leaderboard/.c8rc.json`'s hard thresholds: lines/statements/functions ≥88%, branches ≥60%, scoped to `leaderboard/shared/**`), then (2) `npm run coverage:snapshot -- --check` (dashboard-freshness diff). Step 1 has no `continue-on-error`, so when it fails step 2 never runs. On PR #144 (touches zero leaderboard files) step 1 failed outright: `All files 87.98%` lines/statements vs the `88%` gate — missed by 0.02 points. `shared/auth.js` was the drag (branch coverage 68.57%), but the **actual uncovered lines on that run were 57-63, 68-70, 150-161** — verified against the real file, these are `frontendApiFromPublishableKey()`'s try/catch, `issuerFromEnv()`'s `CLERK_PUBLISHABLE_KEY` fallback branch, and `resolveAuth()`'s ALGO-failure fallthrough `catch` — **none of which touch `getKey()`'s JWKS-TTL check (lines 82-94)**, which the previous note (written after PR #133/#135) blamed. So this isn't one timing-sensitive branch tipping a tight threshold — it's that `auth.js` has *multiple* thin/conditionally-exercised branches, and coverage sits close enough to 88% that whichever subset a given test run happens to (not) hit can tip it either way. Not merge-blocking (non-required check; `mergeable_state` was `unstable` not `blocked` on #135 and #144). Real fix: audit `auth.js` for every branch below ~90% and add a deterministic test per branch (mock `Date.now()` for the JWKS path, supply a malformed publishable key for `frontendApiFromPublishableKey()`, unset `CLERK_ISSUER` with `CLERK_PUBLISHABLE_KEY` set for `issuerFromEnv()`, a non-ALGO error during ALGO auth for the `resolveAuth()` fallthrough) — don't chase a single suspected line, cover the actual gaps each run reveals. |
