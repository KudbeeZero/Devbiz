...
|---|---|---|---|---|---|---|---|---|
| DBZ-060 | Devbiz | TBD | N/A | CI/tests | `PLAN` | N/A | — | **Coverage-gate flake, root cause confirmed 2026-07-07 via PR #144's CI log**: the `leaderboard/shared/ coverage gate` job runs two steps (`.github/workflows/coverage.yml`) — (1) `npm run coverage` (`c8 --check-coverage` against `leaderboard/.c8rc.json`'s hard thresholds: lines/statements/functions ≥88%, branches ≥60%, scoped to `leaderboard/shared/**`), then (2) `npm run coverage:snapshot -- --check` (dashboard-freshness diff). Step 1 has no `continue-on-error`, so when it fails step 2 never runs. On PR #144 (touches zero leaderboard files) step 1 failed outright: `All files 87.98% lines/statements` vs the `88%` gate — missed by 0.02 points. `shared/auth.js` was the drag: branch coverage 68.57%, uncovered lines 57-63/68-70/150-161 — the same `getKey()` JWKS-cache-freshness check (`Date.now() - cached.fetchedAt > JWKS_TTL_MS`) suspected earlier from the *snapshot* step's branch-% drift (72.51% vs 72.64%, PR #133/#135). Same root cause (timing-sensitive branch), two different gates it can trip. Not merge-blocking (non-required check; `mergeable_state` was `unstable` not `blocked` on both #135 and #144), but will keep intermittently red-flagging unrelated PRs until fixed. Fix idea: add a deterministic test that forces both sides of the JWKS-TTL branch (mock `Date.now()`) so it's always covered, rather than loosening the threshold. |
