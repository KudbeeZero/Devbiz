...
| DBZ-062 | Devbiz | #169 | `claude/voidrunner-leaderboard-wiring` | feature | `DRAFT` | Browser preview | — | **Voidrunner Phase B.1 — Wire Online Leaderboard SDK**: Add `GAMES.voidrunner` to `leaderboard/shared/core.js` (score, waveSurvived, bestTime); wire `KD_LB_CONFIG` + SDK in `games/kudbee-voidrunner/index.html`, swap localStorage top-10/initials entry for SDK post/load. Demo mode works immediately. Ref: GAMES_STUDIO_ROADMAP.md Phase B.1. |
| DBZ-064 | Devbiz | #170 | (merged) | feature | `MERGED` | — | Browser preview | **Darts leaderboard wiring — Phase C**: Added `GAMES.darts` metric catalog (score, throws, bestRound); wired `KD_LB_CONFIG` + SDK in `games/kudbee-darts/index.html`; added `_lbInit`/`_lbPostMatch` to `games/kudbee-darts/src/game.js`; fixed `self` variable scoping in `_lbInit`. Local verification passed (syntax clean, demo mode works). Merged 2026-09-22. |
| DBZ-065 | Devbiz | #171 | (merged) | feature | `MERGED` | — | Browser preview | **Phase C leaderboard wiring — 4 games**: Added `GAMES` entries for contra (score, wavesSurvived, fastestTime), munch (score, levelsCleared, fastestTime, bestLevel), orbital (score, wavesSurvived, fastestTime), puzzles (score, bestMoves). Added columns + indexes in `leaderboard/worker/schema.sql`. Fixed `topByMetric` ASC sort and `upsertScore` MIN() in `leaderboard/worker/store-d1.js`. Fixed `sorted()` and `upsertScore()` in `leaderboard/server/store-file.js` for `dir: 'min'`. Fixed inverted `cmp` multiplier. Wired SDK in all 4 games. Test suite passes (105 assertions). Merged 2026-09-22. |
| DBZ-066 | Devbiz | #172 | `claude/pinball-smoothness-fixes` | feature | `DRAFT` | Local verification | — | **Pinball smoothness optimizations**: (1) Physics step guard 600→5 sub-steps (lag spike freeze prevention); (2) Cached `frameNow` eliminates 14 redundant `performance.now()` calls per frame; (3) Idle/gate render throttled 60fps→4fps via `setTimeout`; (4) Bumper glow gradients cached at build time; (5) Early-exit physics sub-step for near-zero velocity balls. Syntax OK, no errors. Draft PR created, awaiting owner review. |
