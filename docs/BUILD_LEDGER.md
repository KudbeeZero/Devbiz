...
|---|---|---|---|---|---|---|---|---|
| DBZ-186 | Devbiz | #186 | `claude/pinball-multiball-stacking-fix` | bugfix | `DRAFT` | local verify 21/21 + 105/105 | Review & merge | **Pinball multiball ejection spread — prevent ball stacking (PR #186):** When multiball erupts from saucers (center or right hole), balls now spread left-right before moving upward instead of clustering. Center saucer ejection: 6-ball → 7-ball spread pattern (angles [-1.2 to 1.2]); spawn distance 60 → 80px; velocity 350 → 400 px/s. Right saucer (most critical): angles [-0.8 to 0.8] → [-1.2 to 1.2] (wider spread); spawn 60 → 80px; velocity 350 → 420 px/s; vertical stagger 30 → 40px; multiball immunity 0.8 → 1.0 seconds. Reduces right-wall stacking and prevents next-shot blocking. Local verification: **21/21** pinball evaluator, **105/105** leaderboard tests. Ready for review. |
| DBZ-182 | Devbiz | #182 | `claude/games-page-professional-polish` | feature | `MERGED` | — | None | **PR182: Games page professional polish** — Comprehensive CSS enhancements for "$1M development team" quality. Game cards: enhanced shadows (multi-layer depth), color-coded gradient overlays, improved badges with glow animations, expanded play button (56px, 1.18 scale hover). Section: improved typography hierarchy, animated kicker/title, better spacing. Hero: 40px+100px shadows, light overlays, animated scanlines. League: backdrop blur, accent glow, responsive padding. Buttons: 12px radius, 18px padding, smooth transitions. Responsive: tablet 2-col, mobile 1-col. Local verification: `npm run verify` — 21/21 + 105/105 passed. All changes CSS-only; no game logic affected. Merged 2026-09-26. |
| DBZ-079 | Devbiz | #183 | `claude/pinball-ball-physics-8khnuy` | bugfix | `MERGED` | — | None | **Pinball stuck-ball recovery & enhanced nudge mechanics (PR #183):** Stuck-ball detection system with three-layer recovery (bounds checking for left/right/ceiling, corner-trap timer, enhanced nudge with adaptive force). Bounds: x < 30 or x > 870 with speed < 50 px/s triggers outward kick; y < 80 with speed < 60 px/s triggers downward push; corner trap (y < 300, x < 200, speed < 40 px/s) uses 2-second timer for randomized escape. Enhanced nudge: detects slow balls (speed < 80 px/s), applies 1.6x multiplier on horizontal force, adds -200 additional downward force. All recovery methods trigger burst particles and "FREED" callouts. Evaluator 21/21, local passed, no CI configured. Merged 2026-09-26. |
| DBZ-181 | Devbiz | N/A | `claude/batch-professional-polish-units` | feature | `MERGED` | — | None | **Batch Professional Polish — All Site Sections (Units 1-7):** Replicated "$1M quality" design patterns from PR #182 (games page) across entire site. Unit 7 (Global CSS): Border-radius scale (--radius-sm/md/lg), shadow architecture (--shadow-sm/md/lg), inset highlights. Unit 1 (Services): Accent color borders, hover glow, multi-effect lift. Unit 2 (Work cards): Dynamic accent RGB per case, shadow escalation, image zoom. Unit 3 (Contact): Layered shadows on form, input focus glows, button consistency. Unit 4 (Doctrine): Article card depth, hover lifts, gradient overlays. Unit 6 (FAQ): Enhanced shadows, smooth transitions, focus states, reduced-motion support. Unit 5 (Pricing) merged separately (DBZ-080). All units merged to main 2026-09-26 (commits 31d7a96..682cfe7). Local verification: `npm run verify` — 21/21 + 105/105 passed. |
