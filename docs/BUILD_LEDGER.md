...
|---|---|---|---|---|---|---|---|---|
| DBZ-079 | Devbiz | #180 | `claude/pinball-ball-physics-8khnuy` | bugfix | `DRAFT` | Manual test (browser play) | Play test & verify fluidity; approve merge | **Pinball ball physics — fluidity fix**: Reduced per-frame velocity damping 0.99975→0.99985 (0.015% vs 0.025% loss) and wall friction 0.018→0.012 (33% reduction). Ball was losing momentum too quickly on contact; friction-pass deduplication already prevents over-damping, so lowered base factors for smoother rolling while maintaining physics. |
