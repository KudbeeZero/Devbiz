...
| `pinball` | **SDK-wired** (merged PR #168): `KD_LB_CONFIG = { GAME: 'pinball' }` + SDK script in `games/kudbee-pinball/index.html`; metrics `score`, `bestMultiball`, `modesCompleted` defined in `leaderboard/shared/core.js`. Game-over overlay shows top-10 + "Post your score" button. Demo mode works immediately. **Ongoing**: smoothness optimizations in PR #172 (physics step guard, cached timestamps, idle throttle, gradient cache, early-exit sub-steps). |
| `darts` | **SDK-wired** (merged PR #170): `KD_LB_CONFIG` + SDK in `games/kudbee-darts/index.html`; `_lbInit`/`_lbPostMatch` in `src/game.js`; metrics `score`, `throws`, `bestRound`. |
| `contra`, `munch`, `orbital`, `puzzles` | **SDK-wired** (merged PR #171): `KD_LB_CONFIG` + SDK added; metrics defined in `leaderboard/shared/core.js`; schema updated with columns + indexes; `store-d1.js` + `store-file.js` fixed for `dir: 'min'`. |
