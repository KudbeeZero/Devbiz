...
(function (KD) {
  ...
  // TIPS change the point geometry + colour/glow. `col:null` means "use the
  // barrel/skin colour" so neon tips theme with the equipped skin.
  const TIPS = {
    steel:  { name: 'Steel Point', col: '#eaf6ff', len: 0.20, glow: 0 },
    needle: { name: 'Needle',      col: '#dfeaff', len: 0.27, glow: 1 },
    neon:   { name: 'Neon Spike',  col: null,      len: 0.21, glow: 7 },
    plasma: { name: 'Plasma Tip',  col: '#ffffff', len: 0.25, glow: 13, hot: true },
    goldp:  { name: 'Gold Point',  col: '#ffd34d', len: 0.20, glow: 4 },
  };

  // FLIGHTS change the tail feather silhouette + opacity. Each `shape` returns
  // the polygon (in barrel-relative units) for one half; the renderer mirrors.
  const FLIGHTS = {
    standard: { name: 'Standard', alpha: 0.92, spread: 1.30, sweep: 0.50, notch: 0 },
    slim:     { name: 'Slim',     alpha: 0.95, spread: 0.85, sweep: 0.50, notch: 0 },
    kite:     { name: 'Kite',     alpha: 0.92, spread: 1.65, sweep: 0.46, notch: 0 },
    shark:    { name: 'Shark',    alpha: 0.94, spread: 1.45, sweep: 0.72, notch: 0 },
    star:     { name: 'Star',     alpha: 0.95, spread: 1.55, sweep: 0.50, notch: 0.4 },
    ghost:    { name: 'Ghost',    alpha: 0.55, spread: 1.70, sweep: 0.50, notch: 0 },
  };
