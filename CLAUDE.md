...
- **2026-09-25** — **Pinball gameplay gate = evaluator 24/24:** before claiming pinball playability merge-ready, run `npm run pinball:eval` from repo root (or `cd games/kudbee-pinball/eval && node evaluator.mjs`). Playwright + Chromium; **24** rubric checks including launch crest / failed-plunge re-serve (no `prepBall` cheat), ur-mini-flip pocket, nudge + Solar Sail, lower-table geo, gutter/flipper-band/drain-lip. Say “local passed, no CI configured.” Leaderboard unit gate: `npm run test:leaderboard`. Leaderboard LIVE is separate — static host 404 on `/api/leaderboard` is expected until Worker routing lands.
...
- **Before starting any task:** verify `main` is locally sound — run the project's
  ...
  sound. Default Devbiz gate from repo root: **`npm run verify`** (pinball 24/24 +
