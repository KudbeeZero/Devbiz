...
- **2026-09-25** — **Pinball gameplay gate = evaluator 20/20:** before claiming pinball playability merge-ready, run `cd games/kudbee-pinball/eval && node evaluator.mjs` (Playwright + Chromium; **20** rubric checks including nudge impulse, lower-table geo, gutter/flipper-band/drain-lip regression). Say “local passed, no CI configured.” Leaderboard LIVE is separate — static host 404 on `/api/leaderboard` is expected until Worker routing lands.
