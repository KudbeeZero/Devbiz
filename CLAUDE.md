...
- **2026-09-26** — **Pinball "ball stuck" → scan the physics, don't patch the spot:** several lanes (DBZ-065/066/067) fixed stuck spots with geometry nudges and timed "FREED" rescues; the real causes were collision bugs (swept-TOI exit root, per-step friction drag, −v̂ normals, flat end-cap normals). Reproduce with `__kbTest.simulate()` over a table-wide grid *before* editing, and never tune constants or geometry without re-running the evaluator — the first attempt on PR #180 shipped unverified flipper changes that created a new wedge. Invariants live in `AGENTS.md` → *Pinball physics rules*. Cloud containers: launch Playwright with `executablePath: '/opt/pw-browsers/chromium'`.
- **2026-09-25** — **Cadence: 2 PR cycles/hour, one open PR max (owner):** timer `kudbee-two-pr-per-hour` @ `delaySeconds: 1800`. Before opening the next PR, merge the current lane to `main` and confirm **zero** open PRs. Details: `/cursor/stores/self/pr-hourly-cadence.md`.
- **2026-09-25** — **Pinball gameplay gate = evaluator 21/21:** before claiming pinball playability merge-ready, run `npm run pinball:eval` from repo root (or `cd games/kudbee-pinball/eval && node evaluator.mjs`). Playwright + Chromium; **21** rubric checks including nudge + Solar Sail mission, lower-table geo, gutter/flipper-band/drain-lip regression. Say “local passed, no CI configured.” Leaderboard unit gate: `npm run test:leaderboard`. Leaderboard LIVE is separate — static host 404 on `/api/leaderboard` is expected until Worker routing lands.
